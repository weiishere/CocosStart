import { DymjGangHuTypeValue } from './DymjGangHuTypeValue';
 ...
 *
 */
export class DymjPlayerGameResult {
    azimuth: number; //玩家方位。
    credit: number; //可用点数。结算后的可用点数。
    shouValues: number[]; //手牌数组。格式：[int]。排好需的手牌。
    huValues: DymjGangHuTypeValue[]; //胡牌数组。需要数组按照玩家“胡”牌顺序排列。格式：
    //		方案一：[int]。如：[5, 3, 7]。
    //		方案二：[{value:int, type:int}]。（暂定方案）
    //			value：胡牌值。
    //			type：胡类型。参看“进行操作”消息。
    gangValues: DymjGangHuTypeValue[]; //杠牌数组。需要数组按照玩家“杠”牌顺序排列。格式：
    //		方案一：[int]。如：[9, 2, 3]。
    //		方案二：[{value:int, type:int}]。（暂定方案）
    //			value：杠牌值。
    //			type：杠类型。参看“进行操作”消息。
