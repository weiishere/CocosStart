...
export default class LeaderEnterprise extends ViewComponent {
    ...
    httpRequest(currentPage) {
        ...
        HttpUtil.send(ApplicationGlobal.BaseUrl + `/api/v1/achievement?userName=${localCacheDataProxy.getLoginData().userName}&pageSize=${this.pageSize}&currentPage=${currentPage}`, res => {
